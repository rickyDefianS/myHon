...
import { getFavoriteBooks } from "../store/actions/actionBook";
...
export default function Favorite() {
  ...
  useEffect(() => {
    dispatch(getFavoriteBooks()).then(() => setIsLoading(false));
  }, []);

  const { favoriteBooks } = useSelector((state) => {
    return state.favoriteBook;
  });

  return (
    <>
      <section className="bg-white dark:bg-gray-900">
        <div className="max-w-screen-xl p-4 py-6 mx-auto mt-28 lg:py-12 md:p-8 lg:p-10">
          {isLoading && (
          ...
          {!isLoading && favoriteBooks && favoriteBooks.length > 0 && (
